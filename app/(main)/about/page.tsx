  ...
  return (
    <div className="space-y-16 py-12">
      ...
      <section className="text-center space-y-4">
        ...
        <p className="text-xl text-primary font-medium">{content?.hero?.nickname || 'Kashi'}</p>
      ...
      <section className="max-w-3xl mx-auto">
        ...
        <div className="prose prose-lg">
          {content?.story?.map((paragraph: any, index: number) => (
            <p key={paragraph.id || index} className="text-foreground-muted leading-relaxed mb-4">
              {paragraph.content}
            </p>
          ))}
        </div>
              ...
              >
                ...
                <ul className="space-y-2">
                  {category.items.map((item: string) => (
                    <li key={item} className="text-foreground-muted flex items-center gap-2">
          ...
          {content?.timeline?.map((item: any, index: number) => (
            ...
            >
              ...
              <div className="bg-card p-6 rounded-2xl border border-border">
                ...
                <p className="text-foreground-muted font-medium mb-2">{item.organization}</p>
        ...
        <div className="flex items-center justify-center gap-4">
          {content?.social?.github && (
            <a
              ...
              className="p-4 bg-card rounded-full border border-border hover:border-primary transition-colors"
            >
